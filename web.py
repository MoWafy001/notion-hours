...
def parseTask(task):
    ...
    if date:
        ...
        if date_start and date_end:
            duration = datetime.fromisoformat(date_end) - datetime.fromisoformat(
                date_start
            )
...
def listTasksDoneToday():
    ...
    today_start_iso_str = (
        datetime.now(tz=current_time_zone)
    ...
    filters = [
        ...
        {"property": "when", "date": {"on_or_after": today_start_iso_str}},
        {"property": "when", "date": {"on_or_before": today_end_iso_str}},
    ]

    response = notion.databases.query(tasks_database_id, filter={"and": filters})
...
def listTasks(goal_id):
    # list tasks records from the tasks database
    filters = [
        {"property": "Goal", "relation": {"contains": goal_id}},
        {
            "property": "status",
...
def startTask(task_id):
    ...
    response = notion.pages.update(
        ...
        properties={
            ...
            "when": {"date": {"start": now_iso, "end": None}},
        },
    )
    return parseTask(response)
        ...
        properties={
            "title": {
                "title": [
                    {
                        "text": {
                            "content": response["properties"]["title"]["title"][0][
                                "plain_text"
                            ]
                        }
                    }
...
def pauseTask(task_id):
    ...
    response = notion.pages.update(
        task_id,
        properties={"when": {"date": {"start": task["date_start"], "end": now_iso}}},
    )
    return parseTask(response)
...
def doneTask(task_id):
    ...
    response = notion.pages.update(
        ...
        properties={
            "status": {"status": {"name": "Done"}},
            "when": {"date": {"start": task["date_start"], "end": now_iso}},
        },
    )
    return parseTask(response)
